...
pub fn start_pipeline_timer(app_handle: tauri::AppHandle) {
  ...
  tauri::async_runtime::spawn(async move {
    loop {
      ...
      let result = run_pipeline(&ai_config, &embedding, &llm, "scheduled", Some(&app_handle)).await;
  ...
  let (status, error_message) = match &result {
    ...
    Err(e) => {
      emit_event(
        app_handle,
        "pipeline:failed",
        serde_json::json!({"run_id": run_id, "error_message": e.to_string()}),
      );
...
/// embedding 启用时的完整流水线：embedding → 去重 → 聚类 → 标题 → 摘要 → 排名 → why_it_matters
async fn execute_with_embedding(
  conn: &mut SqliteConnection,
  ai_config: &AiConfig,
  embedding_provider: &dyn EmbeddingProvider,
  llm_provider: &dyn LLMProvider,
  unprocessed: &[UnprocessedArticle],
  app_handle: Option<&tauri::AppHandle>,
) -> Result<usize, PipelineError> {
  ...
  for (cluster_idx, cluster) in clusters.iter().enumerate() {
    ...
    let signal_title =
      signal_title::generate_signal_title_with_fallback(llm_provider, &titles).await;
...
/// embedding 关闭时的轻量流水线：跳过 embedding/去重/聚类，仅生成摘要 + why_it_matters
async fn execute_without_embedding(
  conn: &mut SqliteConnection,
  _ai_config: &AiConfig,
  llm_provider: &dyn LLMProvider,
  unprocessed: &[UnprocessedArticle],
  ...
  // 为每篇文章创建 ai_analysis 记录（不含 embedding）
  for article in unprocessed {
    diesel::insert_into(article_ai_analysis::table)
      .values((
        article_ai_analysis::article_id.eq(article.id),
        article_ai_analysis::model_version.eq("no-embedding"),
      ))
      .execute(conn)
      .map_err(|e| PipelineError::DbError(e.to_string()))?;
  }
  ...
  emit_progress(app_handle, "ranking", 0, 1);
  for article in unprocessed {
    let content_len = article.description.len() as f64;
    // 基础分：内容越丰富分数越高，范围 0.3 ~ 0.8
    let score = (0.3 + (content_len / 10000.0).min(0.5)) as f32;
    diesel::update(
      article_ai_analysis::table.filter(article_ai_analysis::article_id.eq(article.id)),
    )
...
fn emit_progress(app_handle: Option<&tauri::AppHandle>, stage: &str, current: usize, total: usize) {
  emit_event(
    app_handle,
    "pipeline:progress",
...
pub fn get_signal_detail(
  ...
  let (_, article_id, signal_title, summary, why_it_matters, score) =
    analysis.ok_or_else(|| "Signal not found".to_string())?;
...
pub fn find_related_sources(
  ...
  let target_embedding: Vec<f32> = match embedding_json {
    Some(ref json) if !json.is_empty() => serde_json::from_str(json).unwrap_or_default(),
    _ => {
...
mod tests {
  ...
  fn test_pipeline_error_display() {
    ...
    assert!(PipelineError::DbError("test".to_string())
      .to_string()
      .contains("test"));
  }
